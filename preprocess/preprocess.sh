...
chkfile forward_pe.fastq forward_pe.fasta reverse_pe.fastq reverse_pe.fasta forward_singleton.fastq forward_singleton.fasta reverse_singleton.fastq reverse_singleton.fasta
## Do some work:
cd $SCRATCH

# adds arguments in the fastq_process command string
add_args(){
	
        if [ -s bad_paired_1.fasta ]
        then
		#Copy back bad sequences
		cp bad_paired_1.fasta $SUBMITDIR
                python_args="$python_args --discard_file bad_paired_1.fasta"
		if [ -s bad_paired_2.fasta ]
        	then
			#copy back sequences
			cp bad_paired_2.fasta $SUBMITDIR
...
#First convert forward_unpaired.fastq to fasta (produces the file singelton-file.fasta), and then store reads with low
...
then
	...
	then
		...
		#first rename forward_unpaired.fastq/a
