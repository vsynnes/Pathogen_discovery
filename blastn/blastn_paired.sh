...
cp ../megablast/forward_pe.fasta $SCRATCH
cp ../megablast/forward_pe.fastq $SCRATCH/frw_pe.fastq
cp ../megablast/reverse_pe.fasta $SCRATCH
cp ../megablast/reverse_pe.fastq $SCRATCH/rev_pe.fastq
...
blastn -task blastn  -db blastdb/salmondb -query forward_pe.fasta -evalue 0.0000001 -out forward_removal.txt -outfmt "6 qseqid"
blastn -task blastn  -db blastdb/salmondb -query reverse_pe.fasta -evalue 0.0000001 -out reverse_removal.txt -outfmt "6 qseqid"
...
then
	...
	echo "After BLASTN processing, the dataset contained $new_frw_cnt forward paired end reads, "
...
then
	...
	echo "After BLASTN processing, the dataset contained $new_rev_cnt reverse paired end reads, "
